...
async function ensureDriveReady() {
    const drive = getDriveClient();
    if (!drive) {
        driveLastErrorMessage = 'Chưa có phiên đăng nhập Google Drive hợp lệ. Vui lòng đăng nhập lại Google Drive.';
    ...
    try {
        ...
        console.error('[Drive] Authentication check failed:', error?.message || error);
        return { success: false, error: driveLastErrorMessage };
    }
}
...
async function getPurchaseItemPackaging() {
    if (!prisma) return {};
    const config = await prisma.appConfig.findUnique({ where: { key: PURCHASE_ITEM_PACKAGING_KEY } });
    if (!config?.value) return {};
    try {
        const parsed = JSON.parse(config.value);
        return parsed && typeof parsed === 'object' ? parsed : {};
    ...
    const byLineKey = Object.fromEntries(
        (items || []).map(item => {
            const levels = Array.isArray(item?.packagingLevels)
                ? item.packagingLevels.map(level => ({ id: String(level?.id || ''), name: String(level?.name || ''), factor: Number(level?.factor || 1) }))
                    .filter(level => level.id && level.name && Number.isFinite(level.factor) && level.factor > 0)
                : [];
            const counts = item?.packagingCounts && typeof item.packagingCounts === 'object'
                ? Object.fromEntries(Object.entries(item.packagingCounts).map(([key, value]) => [key, Math.max(0, Number(value || 0))]))
                : {};
            return [getPurchaseItemCompanyKey({ ...item, id: null }), levels.length > 0 ? { packagingLevels: levels, packagingCounts: counts } : null];
        }).filter(([, value]) => value)
    );
    if (Object.keys(byLineKey).length > 0) packaging[String(purchaseId)] = { byLineKey };
    else delete packaging[String(purchaseId)];
    await prisma.appConfig.upsert({
        where: { key: PURCHASE_ITEM_PACKAGING_KEY },
        update: { value: JSON.stringify(packaging) },
        create: { key: PURCHASE_ITEM_PACKAGING_KEY, value: JSON.stringify(packaging) },
    });
}
...
ipcMain.handle('purchases:getAll', async (event, { since, limit } = {}) => {
    try {
        requireRole('admin', 'manager');
        if (!prisma) throw new Error('Prisma not available');
        const [vatGroups, vatFileMeta, purchaseItemCompanies, purchaseItemPackaging, purchaseCompanyVat, goodsCompanies] = await Promise.all([
        ...
        const formatted = purchases.map(p => {
            ...
            const packagingByLine = purchaseItemPackaging[String(p.id)]?.byLineKey || {};
            const itemsFormatted = p.items.map(item => ({
                ...
                ...(packagingByLine[getPurchaseItemCompanyKey({ ...item, id: null })] || {}),
            }));

            const vatGroupMeta = purchaseGroupMeta.get(p.id) || {};
            const fileMeta = vatFileMeta[String(p.id)] || {};
            const companyVatByGroup = Object.fromEntries(
                Object.entries(purchaseCompanyVat[String(p.id)] || {}).map(([company, vat]) => {
...
// Workspace quản lý kiện hàng. Tồn hiện tại luôn lấy từ SKU/variant hiện hành;
// quy cách lấy từ dòng phiếu nhập gần nhất có quy đổi hợp lệ. Không suy diễn
// một quy cách mặc định cho SKU chưa được khai báo trên phiếu nhập.
    ...
    try {
        ...
        const [purchases, packagingStore, dbRegister, registerConfig] = await Promise.all([
            prisma.purchaseOrder.findMany({
                ...
                select: {
                    id: true, poNumber: true, createdAt: true,
                    supplier: { select: { name: true } },
                    items: {
                        select: {
                            id: true, productId: true, quantity: true, color: true, variantSku: true,
                            product: { select: { id: true, name: true, sku: true, unit: true, stock: true, variants: true } }
                        }
                    }
                }
            }),
            getPurchaseItemPackaging(),
            (async () => {
                try { return await prisma.handlingUnit.findMany({ orderBy: { createdAt: 'asc' } }); }
                catch (error) { console.warn('HandlingUnit table is not available yet:', error.message); return []; }
            })(),
            prisma.appConfig.findUnique({ where: { key: 'handlingUnitsRegisterV1' } }),
        ]);
        ...
        purchases.forEach(purchase => {
            ...
            purchase.items.forEach(item => {
                ...
                const packaging = byLine[getPurchaseItemCompanyKey({ ...item, id: null })] || {};
                const rawLevels = Array.isArray(packaging.packagingLevels) ? packaging.packagingLevels : [];
                const levels = rawLevels.map(level => ({
                    id: String(level?.id || ''), name: String(level?.name || '').trim(), factor: Math.max(1, Number(level?.factor || 1))
                })).filter(level => level.id && level.name);
                const base = levels.find(level => level.id === 'lo' || level.name.toLocaleLowerCase('vi-VN') === 'lẻ') || levels.find(level => level.factor === 1);
                if (base) base.factor = 1;
                else levels.unshift({ id: 'lo', name: item.product.unit || 'Lẻ', factor: 1 });
                if (levels.length < 2 || !levels.some(level => level.factor > 1)) return;

                let stock = Number(item.product.stock || 0);
                if (item.variantSku) {
                    const variant = parseJsonArray(item.product.variants).find(entry => String(entry?.sku || '') === String(item.variantSku));
        ...
        let register = dbRegister.map(row => ({
            ...
            status: row.status === 'opened' ? 'Đang sử dụng' : row.status === 'empty' ? 'Đã hết' : 'Nguyên niêm phong',
        ...
        // One-time compatibility migration for records created before the
        // HandlingUnit table was wired to the module.
        if (register.length === 0) {
            try {
                const parsed = JSON.parse(registerConfig?.value || '[]');
                if (Array.isArray(parsed) && parsed.length) {
                    await prisma.handlingUnit.createMany({
                        data: parsed.filter(item => item?.id && catalog.has(String(item?.skuName || ''))).map(item => ({
                            code: String(item.id), productId: Number(item.productId || catalog.get(String(item.skuName)).productId),
                            sku: String(item.skuName), color: item.color || null,
                            packagingName: String(item.packageType || 'Kiện'), baseUnit: String(item.unitName || 'Cái'),
                            conversionFactor: Math.max(1, Number(item.initialPcs || 1)),
                            initialQuantity: Math.max(0, Number(item.initialPcs || 0)), remainingQuantity: Math.max(0, Number(item.currentPcs || 0)),
                            status: item.status === 'Đang sử dụng' ? 'opened' : item.status === 'Đã hết' ? 'empty' : 'sealed',
                            zone: JSON.stringify(item.location || { zone: 'Chưa phân khu' }),
                        })),
                        skipDuplicates: true,
                    });
                    const migrated = await prisma.handlingUnit.findMany({ orderBy: { createdAt: 'asc' } });
                    register = migrated.map(row => ({
                        id: row.code, productId: row.productId, productGroup: catalog.get(row.sku)?.productGroup || 'Sản phẩm',
                        purchaseOrderId: row.purchaseOrderId, purchaseItemId: row.purchaseItemId,
                        variantName: catalog.get(row.sku)?.variantName || row.sku, skuName: row.sku, color: row.color || '',
                        packageType: row.packagingName, packageLabel: `1 ${row.packagingName} (${row.conversionFactor.toLocaleString('vi-VN')} ${row.baseUnit})`,
                        unitName: row.baseUnit, factory: catalog.get(row.sku)?.factory || 'Nhà cung cấp',
                        status: row.status === 'opened' ? 'Đang sử dụng' : row.status === 'empty' ? 'Đã hết' : 'Nguyên niêm phong',
                        location: decodeLocation(row.zone), initialPcs: row.initialQuantity, currentPcs: row.remainingQuantity,
...
ipcMain.handle('handlingUnits:saveRegister', async (_event, records = []) => {
    try {
        ...
        const register = records.map((item) => ({
        ...
        const products = await prisma.product.findMany({ select: { id: true, sku: true, variants: true } });
        const productIdBySku = new Map();
        const stockBySku = new Map();
        products.forEach(product => {
            productIdBySku.set(String(product.sku), product.id);
            stockBySku.set(String(product.sku), Math.max(0, Number(product.stock || 0)));
            try { (JSON.parse(product.variants || '[]') || []).forEach(variant => { productIdBySku.set(String(variant?.sku || ''), product.id); stockBySku.set(String(variant?.sku || ''), Math.max(0, Number(variant?.stock || 0))); }); } catch {}
        });
        ...
        await prisma.$transaction(async tx => {
            ...
            for (const code of existing.map(item => item.code)) {
                if (!incomingCodes.has(code)) await tx.handlingUnit.delete({ where: { code } });
            }
            for (const item of register) {
                const productId = Number(item.productId || productIdBySku.get(item.skuName) || 0);
                if (!productId) throw new Error(`Không xác định được sản phẩm cho SKU ${item.skuName}.`);
                const status = item.status === 'Đang sử dụng' ? 'opened' : item.status === 'Đã hết' ? 'empty' : 'sealed';
                await tx.handlingUnit.upsert({
                    where: { code: item.id },
                    update: { productId, purchaseOrderId: item.purchaseOrderId, purchaseItemId: item.purchaseItemId, sku: item.skuName, color: item.color || null, packagingName: item.packageType || 'Kiện', baseUnit: item.unitName || 'Cái', conversionFactor: Math.max(1, Number(item.initialPcs || 1)), initialQuantity: item.initialPcs, remainingQuantity: item.currentPcs, status, zone: JSON.stringify(item.location || { zone: 'Chưa phân khu' }) },
                    create: { code: item.id, productId, purchaseOrderId: item.purchaseOrderId, purchaseItemId: item.purchaseItemId, sku: item.skuName, color: item.color || null, packagingName: item.packageType || 'Kiện', baseUnit: item.unitName || 'Cái', conversionFactor: Math.max(1, Number(item.initialPcs || 1)), initialQuantity: item.initialPcs, remainingQuantity: item.currentPcs, status, zone: JSON.stringify(item.location || { zone: 'Chưa phân khu' }) },
                });
            }
        });
...
ipcMain.handle('purchases:getMyVatPenaltyAlerts', async () => {
    try {
        ...
        const alerts = purchases.flatMap(purchase => {
            ...
            if (hasVat || firstFineDate > now) return [];
            const daysAfterFirstFine = Math.max(0, Math.floor((now.getTime() - firstFineDate.getTime()) / 86400000));
            ...
            return Array.from({ length: fineStage }, (_, index) => {
                ...
                fineDate.setTime(firstFineDate.getTime());
                fineDate.setDate(fineDate.getDate() + (stage <= 3 ? (stage - 1) * 2 : stage + 1));
                return {
...
ipcMain.handle('purchases:create', async (event, data) => {
    ...
    try {
        ...
        if (!Number.isInteger(supplierId) || supplierId <= 0) {
            throw new Error('Vui lòng chọn nhà cung cấp hợp lệ trước khi tạo phiếu nhập.');
        }
        ...
        const receiptFiles = Array.isArray(data?.importReceiptFiles) ? data.importReceiptFiles : [];
        if (receiptFiles.length === 0) {
            throw new Error('Vui lòng đính kèm ít nhất 1 file Phiếu Nhập Kho trước khi tạo phiếu.');
        }
        if (receiptFiles.some(file => !file || typeof file.fileBase64 !== 'string' || !file.fileBase64.trim() || typeof file.fileName !== 'string' || !file.fileName.trim())) {
            throw new Error('File Phiếu Nhập Kho không hợp lệ. Vui lòng chọn lại file.');
        }
        ...
        if (!receiptFolderId) throw new Error(driveLastErrorMessage || 'Không mở được thư mục Phiếu Nhập Kho trên Google Drive.');
        ...
        for (let index = 0; index < receiptFiles.length; index += 1) {
            ...
            if (!uploaded?.fileId || !uploaded?.webViewLink) {
                throw new Error(`Không thể tải file Phiếu Nhập Kho thứ ${index + 1} lên Google Drive.`);
            }
        ...
        if (uploadedReceiptFileIds.length > 0) {
            try {
                const drive = getDriveClient();
                if (drive) await Promise.all(uploadedReceiptFileIds.map(fileId => drive.files.delete({ fileId }).catch(() => null)));
            } catch (cleanupError) {
                console.error('Could not clean up receipt files after failed purchase create:', cleanupError);
            }
        }
...
ipcMain.handle('purchases:update', async (event, { id, data }) => {
    try {
        ...
        const supplierId = Number(data?.supplierId);
        if (!Number.isInteger(supplierId) || supplierId <= 0) {
            throw new Error('Vui lòng chọn nhà cung cấp hợp lệ trước khi lưu phiếu nhập.');
        }
        ...
        if (!supplier || supplier.status === 'inactive') throw new Error('Nhà cung cấp không tồn tại hoặc đã ngừng sử dụng.');
        ...
        if (nextItems.some(item => !Number.isInteger(Number(item?.productId)) || Number(item?.quantity) <= 0)) {
        ...
        const purchase = await withStockLock(() => getPrismaDirectTx().$transaction(async (tx) => {
            ...
            if (order.status === 'cancelled') throw new Error('Không thể sửa phiếu nhập đã hủy.');
            ...
            if (productMap.size !== productIds.length) throw new Error('Có sản phẩm trong phiếu không còn tồn tại.');

            const stockDeltas = new Map();
            const addStockDelta = (item, multiplier) => {
                const product = productMap.get(Number(item.productId));
                const sku = item.variantSku || product?.sku;
                if (!sku) throw new Error(`Sản phẩm "${product?.name || item.productId}" chưa có SKU.`);
                stockDeltas.set(sku, (stockDeltas.get(sku) || 0) + (Number(item.quantity) * multiplier));
            };
            ...
            await tx.purchaseItem.deleteMany({ where: { purchaseOrderId: order.id } });
        ...
        emitStockChangedForSkus([...new Set(nextItems.map(item => item.variantSku).filter(Boolean))], {
            referenceType: 'NHAP_EDIT',
            reference: purchase.poNumber,
        });
        try {
            ...
            console.error('Could not save updated purchase item packaging:', packagingError);
        }
...
ipcMain.handle('purchases:uploadCompanyVATInvoice', async (event, { purchaseId, companyGroup, invoiceNumber, invoiceDate, files = [] }) => {
    try {
        ...
        if (!vatFolderId) throw new Error(driveLastErrorMessage || 'Không mở được thư mục Hóa đơn VAT trên Google Drive.');
        ...
        for (let index = 0; index < files.length; index += 1) {
            ...
            try {
                ...
                const uploaded = await uploadToDrive(driveStatus.drive, vatFolderId, driveFileName, fileBuffer, ext.toLowerCase() === 'pdf' ? 'application/pdf' : 'image/jpeg');
                if (!uploaded?.webViewLink) throw new Error(`Không tải được file HĐ VAT thứ ${index + 1} lên Google Drive.`);
...
ipcMain.handle('purchases:uploadVATInvoice', async (event, { purchaseId, invoiceNumber, invoiceDate, files = [], fileBase64, fileName }) => {
    try {
        ...
        if (driveUrls.length !== filesList.length) {
            throw new Error(`Google Drive chỉ nhận ${driveUrls.length}/${filesList.length} file Hóa đơn VAT. Phiếu chưa được ghi nhận là đã upload.`);
        }
...
ipcMain.handle('purchases:uploadImportReceipt', async (event, { purchaseId, files = [], fileBase64, fileName }) => {
    try {
        ...
        if (!folderId) throw new Error(driveLastErrorMessage || 'Không mở được thư mục Phiếu Nhập Kho trên Google Drive.');
...
function getDailyTaskEndOfDay(value) {
    ...
    if (Number.isNaN(dueAt.getTime())) throw new Error('Invalid daily task deadline.');
...
async function normalizeOpenDailyTaskDeadlines() {
    ...
    const updates = tasks.flatMap(task => {
        ...
        return task.dueDate.getTime() === normalized.getTime()
            ? []
            : [prisma.dailyTask.update({ where: { id: task.id }, data: { dueDate: normalized } })];
    });
...
function getNextAssignmentWorkingDueDate(dueAt, recurrenceDays) {
    ...
    nextDueAt.setDate(nextDueAt.getDate() + Math.max(0, Math.floor(Number(recurrenceDays) || 0)));
...
async function getCurrentActor() {
    ...
    if (!user || user.status !== 'active') throw new Error('Phiên đăng nhập không hợp lệ.');
    return { id: user.id, username: user.username, fullName: user.fullName || user.username, role: user.role };
}
...
async function createEvidencePenaltyIfDue(task, now = new Date()) {
    ...
    for (const [index, assignee] of recipients.entries()) {
        ...
        try {
            ...
            void logActivity({ module: 'daily_tasks', action: 'PENALTY', recordId: task.id, recordName: task.title, description: `Tự động phạt ${assignee}: ${penalty.amount}đ vì hết ngày chưa nộp bằng chứng`, severity: 'WARNING' });
        } catch (error) {
            throw error;
        }
    }
...
async function cleanupPrematureDailyEvidencePenalties() {
    ...
    const invalidKeys = rows.flatMap(row => {
        try {
            ...
            if (Number.isNaN(dueAt.getTime()) || Number.isNaN(penaltyAt.getTime())) return [];
...
async function cleanupExpiredEvidenceImages() {
    ...
    const tasks = await prisma.dailyTask.findMany({ where: { attachments: { not: null } } });
    for (const task of tasks) {
        const attachments = parseTaskAttachments(task.attachments);
        const evidence = attachments?.evidence;
...
ipcMain.handle('dailyTasks:completeRegularTask', async (_event, taskId, payload = {}) => {
    try {
        const actor = await getCurrentActor();
        const task = await prisma.dailyTask.findUnique({ where: { id: Number(taskId) } });
...
async function repairRecurringAssignmentRestDays() {
    ...
    for (const task of assignments) {
        ...
        const dueAt = new Date(task.dueDate);
        if (recurrenceDays < 1 || Number.isNaN(dueAt.getTime()) || !isDailyTaskPenaltyRestDay(dueAt)) continue;

        const repairedDueAt = getNextAssignmentWorkingDueDate(dueAt, 0);
        await prisma.$transaction([
            prisma.dailyTask.update({ where: { id: task.id }, data: { dueDate: repairedDueAt } }),
            ...
            prisma.appConfig.deleteMany({ where: { key: { startsWith: `${ASSIGNMENT_EVIDENCE_PENALTY_KEY_PREFIX}${task.id}:` } } }),
            prisma.appConfig.deleteMany({ where: { key: { startsWith: `${TASK_PENALTY_KEY_PREFIX}${task.id}:` } } }),
        ]);
        repaired += 1;
    }
    return repaired;
...
async function reconcileRecurringAssignments(now = new Date()) {
    ...
    for (const task of completed) {
        ...
        const nextEvidence = evidence?.required
            ? (({
                submittedAt, submittedBy, submittedUrl, submittedImage, submittedImages,
                reviewedAt, reviewedBy, penaltyCycle, penaltyCount, penaltyEscalation,
                ...config
...
ipcMain.handle('dailyTasks:update', async (event, id, updates) => {
    try {
        ...
        if (updates.dueDate) {
            updateData.dueDate = existingTask.type === 'daily'
...
ipcMain.handle('dailyTasks:delete', async (event, id) => {
    try {
        ...
        await prisma.$transaction([
            ...
            prisma.appConfig.deleteMany({ where: { key: { startsWith: `${TASK_PENALTY_KEY_PREFIX}${taskId}:` } } }),
...
ipcMain.handle('dailyTasks:resetDaily', async () => {
    try {
        ...
        if (lastResetDate === today) {
            ...
            return { success: true, data: { reset: false, deadlineNormalized, message: 'Đã reset hôm nay rồi' } };
        }
        ...
        if (completedTasks.length > 0) {
            ...
            const completionKeys = new Set(
                existingHistory
                    .filter(entry => entry.action === 'completed' || entry.action === 'daily_reset')
                    .map(entry => `${entry.taskId || entry.taskTitle}:${getHistoryDateKey(entry.timestamp)}`)
            );

            // Only add a reset entry when the original confirmation was not saved.
            const newEntries = completedTasks.flatMap(task => {
                const timestamp = task.completedAt ? task.completedAt.toISOString() : lastResetDate || now.toISOString();
...
const CONFIG_ACCESS = Object.freeze({
    ...
    handlingUnitsRegisterV1: { read: ['admin', 'manager'], write: ['admin', 'manager'], sensitive: true },
    attendanceData: { read: ['admin'], write: ['admin'], sensitive: true },
    attendanceDataEmployeeBackup: { read: ['admin'], write: ['admin'], sensitive: true },
...
ipcMain.handle('appConfig:set', async (event, key, value) => {
    try {
        ...
        if (key === 'attendanceData') {
            // Serialize generic attendance saves with employee-profile writes. On
            // every retry we read the newest employee list and preserve it, while
            // still accepting the newest bonuses/fines/config from the caller.
                        ...
                        if (current?.value) {
                            try {
                                ...
                                if (Array.isArray(currentValue?.employees)) {
                                    valueToSave = { ...(value || {}), employees: currentValue.employees };
                                }
                            } catch { }
                        }
            ...
            if (!config) throw lastConflict || new Error('Dữ liệu chấm công đang được cập nhật ở máy khác. Hãy thử lại.');
        } else {
...
function normalizeUntouchedDailyStockCheckScope(sessions) {
    ...
    const normalizedSessions = sessions.map(session => {
        const items = Array.isArray(session?.items) ? session.items : [];
        if (session?.type !== 'daily' || session?.date !== today || session?.status === 'completed'
        ...
        const response = await withStockLock(() => getPrismaDirectTx().$transaction(async (tx) => {
            ...
            for (const raw of rawItems) {
                ...
                if (!stored.stockSnapshotAt) {
                    throw new Error(`SKU ${sku} thuộc phiên kiểm cũ chưa có mốc tồn thực tế. Hãy dùng "Kiểm lại" và nhập lại số trước khi cân bằng.`);
                }
...
ipcMain.handle('stockCheck:getSessions', async () => {
    try {
        ...
        const sessions = await getPrismaDirectTx().$transaction(async (tx) => {
            ...
            const normalizedScope = normalizeUntouchedDailyStockCheckScope(exempted.sessions);
            const toppedUp = await topUpTodayDailyStockCheckProducts(normalizedScope.sessions, tx);
            if (carried.changed || repaired.changed || exempted.changed || normalizedScope.changed || toppedUp.changed) {
...
ipcMain.handle('stockCheck:ensureDailySession', async (event, payload = {}) => {
    try {
        ...
        if (!requestedItems.length) throw new Error('Chưa có SKU để tạo phiên kiểm.');
        ...
        const result = await getPrismaDirectTx().$transaction(async (tx) => {
            ...
            const existing = sessions.find(session => session?.date === today && session?.type === 'daily');
            if (existing) {
                if (carried.changed || repaired.changed) await writeStockCheckSessions(sessions, tx);
                return { session: existing, changed: carried.changed || repaired.changed };
            }
            ...
            if (!managers.length) throw new Error('Chưa có quản lý đang hoạt động để phân công phiên kiểm.');

            const previous = [...sessions]
                .filter(session => session?.type === 'daily' && session?.date < today
                    && managers.some(manager => normalizeStockCheckUsername(manager.username) === normalizeStockCheckUsername(session.assignedTo)))
                .sort((a, b) => String(b.date).localeCompare(String(a.date)))[0];
            const previousIndex = previous
                ? managers.findIndex(manager => normalizeStockCheckUsername(manager.username) === normalizeStockCheckUsername(previous.assignedTo))
                : -1;
            ...
            const items = requestedItems.slice(0, 1000).map(item => {
                ...
                return {
                    ...
                    systemStock: Number.isFinite(Number(item?.systemStock)) ? Number(item.systemStock) : 0,
                    actualStock: null, note: '', difference: 0, balanced: false,
                    requiresNote: false, countLocked: false, retryCount: 0,
                    priorityReason: item?.priorityReason ? String(item.priorityReason) : undefined,
                    priorityLevel: item?.priorityLevel === 'critical' ? 'critical' : item?.priorityLevel === 'high' ? 'high' : undefined,
                };
            }).filter(Boolean);
            ...
            await tx.activityLog.create({ data: {
                module: 'stock_check', action: 'AUTO_CREATE_DAILY_SESSION',
                description: `Tự tạo phiên kiểm ${today}: ${items.length} SKU, giao ${assignee.username}.`,
                recordName: today,
                changes: JSON.stringify({ assignedTo: assignee.username, totalSku: items.length }),
                userName: currentSession.username, severity: 'INFO',
            } });
            return { session, changed: true };
        }, { timeout: 30000, maxWait: 10000 });

        return { success: true, session: sanitizeStockCheckSession(result.session, isPrivilegedStockCheckSession()) };
    } catch (error) {
        return { success: false, error: error.message };
    }
});
    ...
    try {
        ...
        const item = await getPrismaDirectTx().$transaction(async (tx) => {
            ...
            if (!clearCount && storedItem.actualStock === null && !storedItem.balanced) {
                ...
                if (liveStock === null) throw new Error(`Không thể lấy tồn kho hiện tại cho SKU ${payload.sku}.`);
    ...
    try {
        ...
        const result = await withStockLock(() => getPrismaDirectTx().$transaction(async (tx) => {
            ...
            if (!item.stockSnapshotAt) {
                throw new Error('Phiên kiểm cũ chưa có mốc tồn thực tế. Hãy dùng "Kiểm lại" và nhập lại số trước khi cân bằng.');
            }
...
ipcMain.handle('users:update', async (event, id, data) => {
    try {
        ...
        if (data.username !== undefined && String(data.username).trim().toLowerCase() !== existingUser.username.trim().toLowerCase()) {
            const attendanceRecord = await prisma.appConfig.findUnique({ where: { key: 'attendanceData' } });
            let attendanceData = {};
            try { attendanceData = attendanceRecord?.value ? JSON.parse(attendanceRecord.value) : {}; } catch { }
            const linkedToPayroll = (Array.isArray(attendanceData.employees) ? attendanceData.employees : [])
                .some(employee => String(employee?.username || '').trim().toLowerCase() === existingUser.username.trim().toLowerCase());
            if (linkedToPayroll) {
                throw new Error('Không thể đổi username đang liên kết với bảng công/lương.');
            }
        }
        const updateData = {};
...
ipcMain.handle('users:delete', async (event, id) => {
    try {
        ...
        try { attendanceData = attendanceRecord?.value ? JSON.parse(attendanceRecord.value) : {}; } catch { }
        const linkedToPayroll = (Array.isArray(attendanceData.employees) ? attendanceData.employees : [])
            .some(employee => String(employee?.username || '').trim().toLowerCase() === user.username.trim().toLowerCase());
        if (linkedToPayroll) {
            throw new Error('Tài khoản đang liên kết với bảng công/lương. Hãy đánh dấu nghỉ việc thay vì xóa.');
        }
...
ipcMain.handle('attendance:saveEmployeeProfile', async (event, payload = {}) => {
    try {
        requireRole('admin');
        if (!prisma) throw new Error('Prisma not available');

        const mode = payload.mode === 'create' ? 'create' : 'edit';
        const normalizedUsername = String(payload.employee?.username || '').trim().toLowerCase();
        if (!normalizedUsername) throw new Error('Username nhân viên không hợp lệ.');
            ...
            try {
                const result = await prisma.$transaction(async tx => {
                    const [record, account] = await Promise.all([
                        ...
                        tx.user.findFirst({ where: { username: { equals: normalizedUsername, mode: 'insensitive' } } }),
                    ]);
                    if (!account) throw new Error('Username không thuộc tài khoản hiện có trong Quản trị.');

                    let attendanceData = {};
                    try { attendanceData = record?.value ? JSON.parse(record.value) : {}; } catch { }
                    ...
                    if (mode === 'edit') {
                        ...
                        const current = employees.find(employee => Number(employee.id) === employeeId);
                        if (!current) throw new Error('Hồ sơ đã thay đổi ở nơi khác. Hãy tải lại trước khi sửa.');
                        if (String(current.username || '').trim().toLowerCase() !== normalizedUsername) {
                            throw new Error('Không thể đổi username hoặc mã của hồ sơ lương hiện có.');
                        }
                        const updated = {
                            ...current,
                            id: current.id,
                            ...
                            type: payload.employee?.type === 'Seasonal' ? 'Seasonal' : 'Official',
                            baseSalary: Math.max(0, Number(payload.employee?.baseSalary || 0)),
                            isHourly: payload.employee?.type === 'Seasonal',
                            bankId: payload.employee?.bankId || undefined,
                            bankAccount: String(payload.employee?.bankAccount || '').replace(/\s+/g, '') || undefined,
                            bankAccountName: String(payload.employee?.bankAccountName || '').trim().toUpperCase() || undefined,
                        };
                        if (!updated.name) throw new Error('Họ tên nhân viên không được để trống.');
                        nextEmployees = employees.map(employee => Number(employee.id) === employeeId ? updated : employee);
                    } else {
                        if (employees.some(employee => String(employee.username || '').trim().toLowerCase() === normalizedUsername)) {
                            throw new Error('Tài khoản này đã có hồ sơ lương.');
                        }
                        const usedIds = new Set(employees.map(employee => Number(employee.id)));
                        let employeeId = Date.now();
                        while (usedIds.has(employeeId)) employeeId++;
                        const created = {
                            id: employeeId,
                            username: normalizedUsername,
                            name: String(payload.employee?.name || account.fullName || '').trim(),
                            type: payload.employee?.type === 'Seasonal' ? 'Seasonal' : 'Official',
                            baseSalary: Math.max(0, Number(payload.employee?.baseSalary || 0)),
                            isHourly: payload.employee?.type === 'Seasonal',
                            bankId: payload.employee?.bankId || undefined,
                        ...
                        if (!created.name) throw new Error('Họ tên nhân viên không được để trống.');
                    ...
                    await tx.appConfig.upsert({
                        ...
                        create: { key: 'attendanceDataEmployeeBackup', value: JSON.stringify(backup) },
                    });
                    await tx.appConfig.upsert({
                        where: { key: 'attendanceData' },
                        update: { value: JSON.stringify({ ...attendanceData, employees: nextEmployees }) },
                        create: { key: 'attendanceData', value: JSON.stringify({ ...attendanceData, employees: nextEmployees }) },
                    });
                ...
                await logActivity({
                    module: 'attendance',
                    action: mode === 'create' ? 'CREATE_EMPLOYEE_PROFILE' : 'UPDATE_EMPLOYEE_PROFILE',
        ...
        throw lastConflict || new Error('Dữ liệu đang được cập nhật ở máy khác. Hãy thử lại.');
    } catch (error) {
