...
function AppContent() {
    ...
    useEffect(() => {
        // Warm the largest page only after the shell has been idle for a while.
        // Low-memory/data-saving sessions keep the chunk fully on demand.
        if (!actualUser || isAttendanceUiTest || document.visibilityState !== 'visible') return;
        ...
        const preload = () => {
            if (document.visibilityState === 'visible') void loadAttendancePage();
        };
    ...
    // Some operational cards live outside the shell (for example Daily Tasks)
    // and request navigation through a small app-level event.
    useEffect(() => {
        const handleAppNavigate = (event: Event) => {
            const key = String((event as CustomEvent<string>).detail || '').trim();
            if (!key) return;
            navigateTo(key);
        };
    ...
    const hideHeaderTitle = selectedKey === 'business-report'
        || selectedKey === 'stock-check'
        || selectedKey === 'settings'
        || selectedKey === 'ecommerce-export';
    // Dashboard owns its compact summary header and period controls; avoid a
    // second application header row above it.
    const hidePageHeader = selectedKey === 'settings' || selectedKey === 'dashboard' || selectedKey === 'my-profile';
