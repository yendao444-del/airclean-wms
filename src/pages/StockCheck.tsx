...
interface CheckSession {
    ...
    runId?: string;
    date: string;
    type: 'daily' | 'weekend' | 'full' | 'recheck' | 'inspection';
    assignedTo: string;
    assignedName: string;
...
export default function StockCheck({ onExit }: { onExit?: () => void }) {
    ...
    const [inspectionSelectedSkus, setInspectionSelectedSkus] = useState<string[]>([]);
    ...
    useEffect(() => {
        const detectNewDay = async () => {
            ...
            wallDateKeyRef.current = nextDateKey;
            // Only follow midnight automatically when the user was viewing the
            // former "today". Do not interrupt an admin browsing history.
            if (currentDate.format('YYYY-MM-DD') !== previousDateKey) return;

            setCurrentDate(now);
            setActiveSku('');
            setActualUnitCounts({});
            setSealedMismatchUnits({});
            ...
            try {
                ...
                message.error(error?.message || 'Không thể tải phiên kiểm của ngày mới.');
            }
        };

        const timer = window.setInterval(() => { void detectNewDay(); }, 30000);
        const handleVisibility = () => {
            if (document.visibilityState === 'visible') void detectNewDay();
        };
    ...
    const dailyDisabledByFull = !TEMPORARY_DAILY_ONLY_MODE && Boolean(fullSessionForDate);
    ...
    const inspectionSessionsForDate = sessions
        .filter(session => session.date === todayStr && (session.type === 'inspection' || session.type === 'recheck') && session.status !== 'cancelled')
        .sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
    const todaySession = activeTab === 'inspection'
        ? (inspectionSessionsForDate.find(session => session.id === selectedInspectionId) || inspectionSessionsForDate[0])
    ...
    const getMismatchItems = (session: CheckSession) => session.items.filter(item =>
        item.verificationStatus === 'balanced_mismatch' || Number(item.difference || 0) !== 0
    );

    const inspectionCatalog = useMemo(() => {
        const bySku = new Map<string, CheckItem>();
        contextProducts.flatMap(expandToVariants).forEach(item => {
            const sku = String(item.sku || '').trim();
            if (sku && !bySku.has(sku)) bySku.set(sku, item);
        });
        return [...bySku.values()].sort((a, b) =>
            a.productName.localeCompare(b.productName, 'vi') || String(a.color || '').localeCompare(String(b.color || ''), 'vi')
        );
    }, [contextProducts]);

    const filteredInspectionCatalog = useMemo(() => {
        const query = normalizeUserText(inspectionSearch);
        if (!query) return inspectionCatalog;
        return inspectionCatalog.filter(item =>
    ...
    }, [inspectionCatalog, inspectionSearch]);

    const openInspectionDrawer = (source?: CheckSession) => {
        const suggestedItems = source
            ? (getMismatchItems(source).length > 0 ? getMismatchItems(source) : source.items)
            : [];
        const preferredAssignee = assignableManagers.find(staff => staff.username === source?.assignedTo) || assignableManagers[0];
        setInspectionSearch('');
        setInspectionSelectedSkus(suggestedItems.map(item => item.sku));
        setInspectionAssignee(preferredAssignee?.username || '');
        setInspectionReason(source ? 'Kiểm xác minh lại kết quả của phiên trước.' : '');
        setInspectionDrawerOpen(true);
    };

    const createInspectionSession = async () => {
        if (!inspectionAssignee || !inspectionReason.trim() || !inspectionSelectedSkus.length) {
            message.warning('Chọn SKU, người phụ trách và nhập lý do tạo phiếu kiểm.');
            return;
        }
        setCreatingInspection(true);
        try {
            const result = await window.electronAPI.stockCheck.createInspectionSession({
                skus: inspectionSelectedSkus,
                assignedTo: inspectionAssignee,
            ...
            setSealedMismatchUnits({});
            setExpandedProductGroups({});
            message.success(`Đã tạo phiếu kiểm gồm ${inspectionSelectedSkus.length} SKU.`);
        } catch (error: any) {
            message.error(error?.message || 'Không thể tạo phiếu kiểm.');
        } finally {
    ...
    const assignableManagers = useMemo(() =>
        staffList
            .filter(staff =>
                isStockCheckAssignee(staff)
                && !STOCK_CHECK_EXCLUDED_ASSIGNEE_USERNAMES.has(staff.username.trim().toLowerCase())
            )
            .sort((a, b) => a.username.localeCompare(b.username, 'vi')),
        [staffList]
    );
    ...
    // Saturday is reserved for a warehouse-wide full count.
    useEffect(() => {
        if (!isToday || !weekend || !fullWindowOpen || !sessionsLoaded || !contextProducts.length) return;
        if (user?.role !== 'admin' && user?.role !== 'manager') return;
        if (sessions.some(session => session.date === todayStr && session.type === 'full' && session.status !== 'cancelled')) return;
        if (autoGeneratedSessionRef.current === `weekend-full-${todayStr}`) return;

        const items = contextProducts.flatMap((product: any) => expandToVariants(product));
        const assignee = pickNextAssignee();
        if (!items.length || !assignee) return;

        autoGeneratedSessionRef.current = `weekend-full-${todayStr}`;
        void window.electronAPI.stockCheck.createFullSession({ items, assignedTo: assignee.username }).then(result => {
            if (!result?.success || !result.session) {
                autoGeneratedSessionRef.current = null;
                message.error(result?.error || 'Không thể tự tạo phiên kiểm toàn bộ cuối tuần.');
                return;
            }
            setSessions(previous => [...previous.filter(session => session.id !== result.session.id), result.session]);
        }).catch(error => {
            autoGeneratedSessionRef.current = null;
            message.error(error?.message || 'Không thể tự tạo phiên kiểm toàn bộ cuối tuần.');
        });
    ...
    const handleGenerate = async () => {
        ...
        if (!useFullInventory && weekend) {
            message.info('Thứ 7 chỉ mở phiên kiểm toàn bộ cho tất cả kho.');
            return;
        }
    ...
    return (
        <div style={{ background: '#F8FAFC', minHeight: '100vh' }}>
            <main style={{ width: '100%', maxWidth: 'none', margin: 0, padding: '16px 28px 0' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        {activeTab === 'inspection' && inspectionSessionsForDate.length > 1 && (
                            <Select
                                ...
                                style={{ width: 340 }}
                                options={inspectionSessionsForDate.map(session => ({
                                    value: session.id,
                                    label: `${session.type === 'recheck' ? 'Phiếu cũ' : dayjs(session.createdAt).format('HH:mm')} · ${session.items.length} SKU · ${resolveAssigneeLabel(session)}`,
                                }))}
                            />
                        )}
                        {activeTab === 'inspection' && canCreateInspection && isToday && (
                            <Button
                                type="primary"
                                icon={<PlusOutlined />}
                                onClick={() => openInspectionDrawer()}
                ...
                {activeTab === 'inspection' && todaySession && (
                    <Alert
                        type="info"
                        showIcon
                    ...
                    <div style={{ textAlign: 'center', padding: '80px 0', color: '#94a3b8' }}>
                        {isPast ? (
                            ...
                            <>
                                    ...
                                    // Chưa có assignment nào (hiếm, thường staff chưa load xong)
                                     <>
                                         ...
                                         <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>
                                             {activeTab === 'inspection' ? 'Chưa có phiếu kiểm cho ngày này' : 'Chưa có phiên kiểm cho ngày này'}
                                         </div>
                                         <div style={{ fontSize: 13, marginBottom: 24 }}>
                                             {activeTab === 'full'
                                ...
                                {(canManage || activeTab === 'full') && activeTab !== 'inspection' && (
                                    <Button
                                ...
                                {activeTab === 'inspection' && !todaySession && canCreateInspection && (
                                    <Button
                                        type="primary"
                                        size="large"
                                        icon={<PlusOutlined />}
                                        onClick={() => openInspectionDrawer()}
                                        style={{ marginTop: 8, background: '#10b981', borderColor: '#10b981', borderRadius: 12, fontWeight: 700, height: 44, padding: '0 32px' }}
                                    >
                                        Tạo phiếu kiểm
                                    </Button>
                                )}
            ...
            <Drawer
                title={
                    <div>
                        <div style={{ fontSize: 19, fontWeight: 800, color: '#0f172a' }}>Tạo phiếu kiểm</div>
                        <div style={{ marginTop: 4, fontSize: 12, color: '#64748b', fontWeight: 500 }}>Chọn đúng SKU cần kiểm thực tế</div>
                    </div>
                }
                ...
                footer={
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                        <Button onClick={() => setInspectionDrawerOpen(false)}>Hủy</Button>
                        <Button
                            type="primary"
                            icon={<PlusOutlined />}
                            loading={creatingInspection}
                            disabled={!inspectionSelectedSkus.length || !inspectionAssignee || !inspectionReason.trim()}
                            onClick={() => { void createInspectionSession(); }}
                            style={{ background: '#10b981', borderColor: '#10b981', fontWeight: 700 }}
                        >
                            Tạo phiếu kiểm ({inspectionSelectedSkus.length} SKU)
                        </Button>
                    </div>
                }
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div>
                        <div style={{ marginBottom: 7, fontWeight: 800, color: '#334155' }}>Sản phẩm / SKU cần kiểm <span style={{ color: '#ef4444' }}>*</span></div>
                        <Input.Search
                            allowClear
                            value={inspectionSearch}
                            onChange={event => setInspectionSearch(event.target.value)}
                            placeholder="Gõ tên sản phẩm, SKU hoặc màu..."
                            style={{ marginBottom: 10 }}
                        />
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
                            <Checkbox
                                checked={filteredInspectionCatalog.length > 0 && filteredInspectionCatalog.every(item => inspectionSelectedSkus.includes(item.sku))}
                                indeterminate={filteredInspectionCatalog.some(item => inspectionSelectedSkus.includes(item.sku)) && !filteredInspectionCatalog.every(item => inspectionSelectedSkus.includes(item.sku))}
                                onChange={event => {
                                    const visibleSkus = new Set(filteredInspectionCatalog.map(item => item.sku));
                                    setInspectionSelectedSkus(current => event.target.checked
                                        ? [...new Set([...current, ...visibleSkus])]
                                        : current.filter(sku => !visibleSkus.has(sku))
                                    );
                                }}
                            >
                                Chọn toàn bộ kết quả ({filteredInspectionCatalog.length})
                            </Checkbox>
                            <Tag color="green">Đã chọn {inspectionSelectedSkus.length} SKU</Tag>
                        </div>
                        <Table<CheckItem>
                            size="small"
                            rowKey="sku"
                            dataSource={filteredInspectionCatalog}
                            pagination={{ pageSize: 20, size: 'small', showSizeChanger: false }}
                            scroll={{ y: 300, x: 620 }}
                            rowSelection={{
                                selectedRowKeys: inspectionSelectedSkus,
                                preserveSelectedRowKeys: true,
                                onChange: keys => setInspectionSelectedSkus(keys.map(String)),
                            }}
                            onRow={item => ({
                                onClick: () => setInspectionSelectedSkus(current =>
                                    current.includes(item.sku) ? current.filter(sku => sku !== item.sku) : [...current, item.sku]
                                ),
                                style: { cursor: 'pointer' },
                            })}
                            locale={{ emptyText: inspectionSearch ? 'Không tìm thấy sản phẩm hoặc SKU phù hợp' : 'Chưa có sản phẩm trong danh mục' }}
                            columns={[
                                { title: 'Sản phẩm', dataIndex: 'productName', width: 250, ellipsis: true, render: value => <strong>{value}</strong> },
                                { title: 'Phân loại', dataIndex: 'color', width: 120, render: value => value ? <Tag>{value}</Tag> : '—' },
                                { title: 'SKU', dataIndex: 'sku', width: 190, ellipsis: true, render: value => <span style={{ color: '#1677ff', fontSize: 12, fontWeight: 700 }}>{value}</span> },
                            ]}
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            value={inspectionAssignee || undefined}
                            onChange={setInspectionAssignee}
                            placeholder="Chọn người phụ trách"
                            style={{ width: '100%' }}
                            options={assignableManagers.map(staff => ({ value: staff.username, label: `${staff.fullName} (${staff.username})` }))}
                        />
                    </div>
                    <div>
                        <div style={{ marginBottom: 7, fontWeight: 800, color: '#334155' }}>Lý do kiểm <span style={{ color: '#ef4444' }}>*</span></div>
                        <Input.TextArea
                            rows={3}
                            maxLength={500}
                            showCount
                            ...
                            placeholder="Ví dụ: Kiểm xác minh tồn thực tế của sản phẩm..."
                        />
                    </div>
                    <Alert
                        type="info"
                        showIcon
                        message={`Phiếu được tạo cho hôm nay ${dayjs().format('DD/MM/YYYY')}`}
                        description="Phiếu này độc lập, không ảnh hưởng lịch kiểm hàng ngày, vòng phân công hoặc chính sách phạt. Tồn hệ thống được lấy tại thời điểm người phụ trách bắt đầu nhập thực tế."
                    />
                </div>
            </Drawer>
        </div>
