...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        }, 60000); // ⚡ 60s + chỉ refresh khi tab active
    ...
    const loadData = async (silent = false) => {
        try {
            ...
            const [pRes, exRes, ecRes, puRes] = await Promise.all([
                api.products.getAll(),
    ...
    const revenue = filteredExports.reduce((s, e) => s + (e.totalAmount || 0), 0) + filteredEcom.reduce((s, e) => s + (e.totalAmount || 0), 0);
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
                ...
                <div style={{
                    ...
                    <div style={{ fontSize: 28, fontWeight: 800, color: profit >= 0 ? '#059669' : '#dc2626', lineHeight: 1.2 }}>{fmtShort(profit)}<span style={{ fontSize: 14, fontWeight: 500 }}>đ</span></div>
                    <div style={{ marginTop: 8 }}>
                        <span style={{ fontSize: 11, color: '#9ca3af' }}>Biên LN: {revenue > 0 ? ((profit / revenue) * 100).toFixed(1) : 0}%</span>
                    </div>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
                ...
                <div style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', border: '1px solid #f3f4f6' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                        <span style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>Doanh thu 7 ngày</span>
                            ...
                            {topProducts.slice(0, 8).map((p, i) => {
                                ...
                                return (
                                    <div key={p.name} style={{
                                        ...
                                        <span style={{ textAlign: 'right', fontSize: 14, fontWeight: 800, color: '#111827' }}>
                                            {p.qty}
                                        </span>
                                        <span style={{ textAlign: 'right', fontSize: 11, fontWeight: 500, color: '#6b7280' }}>
                ...
                <div style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', border: '1px solid #f3f4f6' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: 15, fontWeight: 800, color: '#059669' }}>{fmtShort(filteredExports.reduce((s, e) => s + (e.totalAmount || 0), 0))}đ</div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: 15, fontWeight: 800, color: '#2563eb' }}>{fmtShort(filteredEcom.reduce((s, e) => s + (e.totalAmount || 0), 0))}đ</div>
                        </div>
                    </div>

                    {/* Progress: POS vs TMDT */}
                    {orderCount > 0 && (
