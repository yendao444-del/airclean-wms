...
interface LockedPeriod {
    ...
    lockedAt: string;
    lockedBy: string;
    payrollSnapshot?: LockedPayrollSnapshot;
}

interface LockedPayrollSnapshot {
    version: 1;
    capturedAt: string;
    capturedBy: string;
    rows: any[];
    packingLogs: PackingOrderLog[];
    fines: FineRecord[];
    bonuses: BonusRecord[];
    sourceSummary: {
...
interface PackingOrderLog {
    ...
    orderCount?: number;       // Compact payroll rows can represent multiple orders
...
const isAugust2026WaivedFine = (fine: FineRecord) => {
    ...
    const isLateFine = normalizeAttendanceText(fine.type).includes('di muon');
    return isInAugust && !isLateFine;
};
...
// Compare the selected period with the equivalent elapsed slice immediately
// before it. For an active week, Monday–today is compared with the previous
// week's Monday–same weekday, so future days cannot create a false decrease.
const getPackingComparisonRange = (range: AttendancePeriod): AttendancePeriod => {
    const now = dayjs();
    const periodDays = Math.max(range[1].startOf('day').diff(range[0].startOf('day'), 'day') + 1, 1);
    const observedEnd = range[1].isAfter(now, 'day') ? now.endOf('day') : range[1].endOf('day');
    const elapsedDays = Math.min(periodDays, Math.max(1, observedEnd.diff(range[0].startOf('day'), 'day') + 1));
    const comparisonEnd = range[0].subtract(1, 'day').endOf('day');
...
function buildPackingWeeklyResults(orderLogs: PackingOrderLog[], employeesList: Employee[], asOf: dayjs.Dayjs): PackingWeeklyResult[] {
    ...
    orderLogs.forEach(order => {
        ...
        current.orderCount += Math.max(1, Number(order.orderCount || 1));
...
function calculatePayroll(
    ...
    orderLogs.forEach(order => {
        ...
        matchedEmployees.forEach(employee => {
            ...
            aggregate.orderCount += Math.max(1, Number(order.orderCount || 1));
...
const FaceAttendanceTab = forwardRef<FaceAttendanceTabHandle, {
    employees: any[],
    systemUsers?: any[],
    children?: React.ReactNode,
    ...
    const handleRegister = useCallback(async () => {
        ...
        const linkedAccount = systemUsers.find((account: any) =>
            normalizeAttendanceText(account?.username) === normalizeAttendanceText(matchedEmployee?.username || regFaceId)
            || normalizeAttendanceText(account?.fullName) === normalizeAttendanceText(regUserName)
        );
    ...
    useImperativeHandle(ref, () => ({ toggleCamera, openRegister }), [openRegister, toggleCamera]);
    ...
    return (
        <div className="att-face-layout">
            ...
            <Drawer
                title={<Space><UserOutlined style={{ color: '#00ab56' }} /><Text strong>Quản lý khuôn mặt ({profiles.length})</Text></Space>}
            ...
            >
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                    {isAdmin && (
                        <Button type="primary" block icon={<PlusOutlined />} disabled={serviceStatus !== 'ready'} onClick={openRegister}>
                    ...
                    {profiles.length === 0 ? (
                        ...
                        <div key={p.faceId} className="att-face-profile-row">
                            <div>
            ...
            <section className="att-attendance-achievement" aria-label="Thành tích điểm danh">
...
const AttendancePeriodSelector = ({
    ...
    const thisWeekStart = getPackingWeekStart(now);
    const thisWeekEnd = thisWeekStart.add(6, 'day').endOf('day');
    const previousWeekStart = thisWeekStart.subtract(7, 'day');
    const previousWeekEnd = previousWeekStart.add(6, 'day').endOf('day');

    const presets = [
        { label: 'Tuần này', value: [thisWeekStart, thisWeekEnd] as AttendancePeriod },
        { label: 'Tuần trước', value: [previousWeekStart, previousWeekEnd] as AttendancePeriod },
        { label: 'Tháng này', value: [now.startOf('month'), now.endOf('month')] as AttendancePeriod },
        { label: 'Tháng trước', value: [now.subtract(1, 'month').startOf('month'), now.subtract(1, 'month').endOf('month')] as AttendancePeriod },
        { label: 'Trong 7 ngày qua', value: [now.subtract(6, 'day').startOf('day'), now.endOf('day')] as AttendancePeriod },
        { label: 'Trong 30 ngày qua', value: [now.subtract(29, 'day').startOf('day'), now.endOf('day')] as AttendancePeriod },
    ];

    return (
        <div className="att-header-period">
            <DatePicker.RangePicker
                className="att-header-period__picker"
                value={[start, end]}
                presets={presets}
                format="DD/MM/YYYY"
...
export default function Attendance() {
    ...
    const [attendanceRewardSummaries, setAttendanceRewardSummaries] = useState<AttendanceRewardSummary[]>([]);
    ...
    const fetchImageAsDataUrl = async (url: string, timeoutMs = 4000) => {
    ...
    const buildPayslipPDF = async (payslipData?: any) => {
        ...
        try {
            ...
            while (renderedPx < canvas.height) {
                ...
                pdf.addImage(imgData, 'JPEG', margin, margin, maxW, imgH, undefined, 'FAST');
            ...
            console.log(`[PAYSLIP] Rendered ${data.name} in ${Math.round(performance.now() - startedAt)}ms`);
    ...
    const handleSendPayslipGmail = async () => {
        ...
        try {
            ...
            if (!isValidEmailAddress(recipientEmail)) {
                message.error(`Email của ${payslipModal.name} không hợp lệ: ${recipientEmail}. Vui lòng sửa trong Cài đặt > Quản trị.`);
                return;
            }
            await waitForPaint();
            const result = await buildPayslipPDF();
            ...
            if (res?.success) {
                ...
                message.success(`Gmail đã tiếp nhận phiếu lương gửi tới ${recipientEmail}. Thư trả lại (nếu có) sẽ xuất hiện sau trong hộp thư Gmail.`);
            ...
            onOk: async () => {
                ...
                for (let i = 0; i < toSend.length; i++) {
                    ...
                    try {
                        ...
                        if (!isValidEmailAddress(recipientEmail)) {
                            results.push({ name: p.name, success: false, error: `Email không hợp lệ: ${recipientEmail}` });
                            continue;
                        }
                        setPayslipModal(p);
                        await waitForPaint();
                        ...
                        const pdfBytes = new Uint8Array(pdf.output('arraybuffer'));
                        setBulkSendProgress(prev => prev ? { ...prev, currentName: `Đang chuyển Gmail: ${p.name}` } : prev);
                        await new Promise<void>(resolve => window.setTimeout(resolve, 0));
                        const res = await api.attendance.sendPayslipEmail({ to: recipientEmail, employeeName: p.name, period, fileName, pdfBytes });
    ...
    const payrollLoadStartedAt = useRef(performance.now());
    ...
    useEffect(() => {
        ...
        // Catalog is independent from the attendance snapshot. Start it on
        // the first event-loop turn so it can overlap the core DB read.
        ...
        const loadData = async () => {
            ...
            try {
                ...
                if (rs && rs.success && rs.data) {
                    ...
                    if (!cancelled) setIsCoreSnapshotReady(true);
                } else {
    ...
    const mergeAttendanceSnapshotWithDb = useCallback(async (snapshot: Record<string, any>) => {
        ...
        try {
            ...
            if (!latest?.success) {
                throw new Error(latest?.error || 'Không đọc được attendanceData mới nhất trước khi lưu.');
            }
            ...
            return {
                ...
                // Payroll locks are changed only through the atomic lock API.
                // An old open renderer must never overwrite a newer lock state.
                lockedPeriods: dbData.lockedPeriods ?? safeSnapshot.lockedPeriods ?? [],
            };
        } catch (error) {
            console.error('Dừng autosave vì không xác nhận được trạng thái khóa mới nhất:', error);
            throw error;
        }
    }, []);
    ...
    useEffect(() => {
        ...
        const refreshFineLedger = async () => {
            try {
                const api = (window as any).electronAPI;
                const latest = await api.appConfig.get('attendanceData');
                if (cancelled || !latest?.success || !latest.data) return;
                const dbFines = Array.isArray(latest.data.extraFines) ? latest.data.extraFines : [];
                const dbAuditLog = Array.isArray(latest.data.fineAuditLog) ? latest.data.fineAuditLog : [];
                ...
                const mergedFines = mergeFinesWithDeletes(
                    dbFines.map((fine: FineRecord, index: number) => ensureFineId(fine, index)),
    ...
    useEffect(() => {
        ...
        const syncTask = (async () => {
            ...
            // gates payroll readiness, so do not add an artificial 700ms wait.
            if (cancelled) return;
            const api = (window as any).electronAPI;
            try {
    ...
    const activeDateRange = activeTab === 'packaging' ? packingDateRange : overviewDateRange;

    // Fast Refresh có thể giữ lại state tuần từ phiên bản cũ, dù mặc định mới
    // của các tab ngoài Đóng gói đã là tháng. Chỉ chuyển đúng state cũ này.
    useEffect(() => {
        const now = dayjs();
        const oldWeekStart = getPackingWeekStart(now);
        const oldWeekEnd = oldWeekStart.add(6, 'day').endOf('day');
    ...
    const attendancePeriodSelector = useMemo(() => (
        <AttendancePeriodSelector
            ...
            onChange={activeTab === 'packaging' ? setPackingDateRange : setOverviewDateRange}
        />
    ...
    const loadSalesBonusSummary = useCallback(async (
        ...
        // Do not reset summary here: it is a callback dependency, so resetting
        // it retriggers the loading effect before this request can finish.
        // The period readiness key masks the previous result while loading.
    ...
    const loadPackingOrders = async (since?: string, options?: { strict?: boolean; range?: AttendancePeriod; includeComparison?: boolean; silent?: boolean; force?: boolean; summaryOnly?: boolean }): Promise<PackingOrderLog[]> => {
        ...
        try {
            ...
            if (options?.summaryOnly && api.ecommerceExports.getPackingPayrollSummary) {
                const summaryRes = await withTimeout(
                    api.ecommerceExports.getPackingPayrollSummary({
                        since: sinceVal,
                        until: untilVal,
                        commission: packingCommission,
                    }),
                    'Packing payroll summary',
                );
                if (summaryRes?.success && Array.isArray(summaryRes.data)) {
                    console.log('[PACKING] Payroll summary rows:', summaryRes.data.length, 'cached:', Boolean(summaryRes.cached));
                    return commitPackingRows(summaryRes.data as PackingOrderLog[]);
                }
                throw new Error(summaryRes?.error || 'Không tải được tổng hợp đóng gói cho bảng lương.');
            }
            ...
            while (ecRes.hasMore && ecommerceRows.length < 50000) {
                ...
                if (!ecRes?.success || !Array.isArray(ecRes.data)) {
                    throw new Error('Không tải đủ các trang dữ liệu đóng gói. Dữ liệu gần nhất được giữ nguyên để tránh tính thiếu thưởng.');
                }
                if (ecRes.hasMore && ecRes.data.length === 0) {
                    throw new Error('Phân trang dữ liệu đóng gói không tiến triển. Chưa chốt để tránh tính thiếu thưởng.');
                }
                ecommerceRows.push(...ecRes.data);
            }
            if (ecRes.hasMore) {
                throw new Error('Dữ liệu đóng gói của kỳ vượt 50.000 đơn. Chưa chốt để tránh tính thiếu thưởng.');
            }
            ...
            const processOrder = (order: any, source: string) => {
                ...
                unified.push({
                    id: `${source}-${order.id}`,
                    timestamp: String(order.ecommerceExportDate || order.exportDate || order.date || order.createdAt || order.updatedAt || ''),
    ...
    const attendanceRewardPeriodKey = `${selectedYear}-${String(selectedMonth).padStart(2, '0')}`;
    ...
    useEffect(() => {
        ...
        const loadAttendanceRewardSummary = async () => {
            ...
            try {
                const result = await api.attendance.getRewardSummary(attendanceRewardPeriodKey);
                if (cancelled) return;
                if (result?.success && result.data) {
                    setAttendanceRewardSummaries(Array.isArray(result.data.summaries) ? result.data.summaries : []);
                    setAttendanceRewardConfig(result.data.config || null);
                } else {
                    setAttendanceRewardSummaries([]);
                }
            } catch (error) {
                if (!cancelled) {
                    console.error('Lỗi tải thành tích chuyên cần:', error);
                    setAttendanceRewardSummaries([]);
                }
            } finally {
                if (!cancelled) setAttendanceRewardReadyKey(attendanceRewardPeriodKey);
    ...
    // also catches packing completed from another workstation.
    useEffect(() => {
        if (!isDbLoaded || activeTab !== 'packaging') return;
        const api = (window as any).electronAPI;
        if (!api?.ecommerceExports?.getPackingRevision) return;
        ...
        const runRefresh = async () => {
            ...
            refreshRunning = true;
            try {
                // If the first tab load is still running, wait for it and then
                // query again so a scan committed mid-request cannot be missed.
                const existingLoad = loadPackingPromiseRef.current?.promise;
                if (existingLoad) {
                    try {
                        await existingLoad;
                    } catch {
                        // The silent retry below keeps the last good dashboard data.
                    }
                    if (disposed) return;
                }
                await loadPackingOrders(undefined, {
        ...
        const requestRefresh = (delay = 120) => {
            if (disposed) return;
        ...
        const checkRevision = async () => {
            ...
            try {
                ...
                const nextRevision = String(result.data.revision);
    ...
    function inPackingRange(dateStr?: string) {
        if (!dateStr) return true;
        const d = dayjs(dateStr);
        return d.isAfter(packingDateRange[0].startOf('day').subtract(1, 'ms'))
    ...
    const liveOverviewAttendanceBonuses = useMemo(() => attendanceRewardSummaries
        ...
        .map(summary => ({
            ...
            type: 'Thưởng chuyên cần tháng',
            detail: `Đúng giờ ${summary.monthly.onTimeDays}/${summary.monthly.targetDays} ngày trong tháng ${summary.periodKey}`,
            amount: summary.monthly.rewardAmount,
            date: `${summary.monthly.qualifiedAt || `${summary.periodKey}-01`}T23:59:59+07:00`,
            createdBy: 'system',
    ...
    const liveOverviewBonusesWithWeekly = useMemo(() => {
        ...
        const allIds = new Set([...manualIds, ...weekly.map(bonus => bonus.id)]);
    ...
    const activeSalesBonusSummary = currentLockedPeriod
        ...
        : salesBonusReadyKey === `${overviewDateRange[0].startOf('day').valueOf()}-${overviewDateRange[1].endOf('day').valueOf()}`
            ? salesBonusSummary
            : EMPTY_SALES_BONUS_SUMMARY;
    ...
    const packingDashboardData = useMemo(() => {
        ...
        const matchedEmployeesByPacker = new Map<string, Employee[]>();
        const previousOrderCounts = new Map<number, number>();

        livePackingTrendLogs.forEach(order => {
            const packerKey = String(order.packer || '');
            let matchedEmployees = matchedEmployeesByPacker.get(packerKey);
            if (!matchedEmployees) {
            ...
            matchedEmployees.forEach(employee => {
            previousOrderCounts.set(employee.id, (previousOrderCounts.get(employee.id) || 0) + Math.max(1, Number(order.orderCount || 1)));
            });
        });

        livePackingLogs.forEach(order => {
            ...
            matchedEmployees.forEach(employee => {
                ...
                if (!score) return;
                score.orderCount += Math.max(1, Number(order.orderCount || 1));
    ...
    useEffect(() => {
        ...
        console.info('[Attendance:ready]', JSON.stringify({
            elapsedMs: Math.round(performance.now() - payrollLoadStartedAt.current),
            ready: isPayrollDataReady,
            core: isCoreSnapshotReady,
            attendance: overviewAttendanceReady,
            fines: areFineSourcesReady,
            catalog: packingCatalogReady,
            packing: isPackingDataReady,
            rewards: attendanceRewardReadyKey === attendanceRewardPeriodKey,
    ...
    const payrollData = useMemo(() => {
        ...
        const rows = calculatePayroll(
            ...
            overviewAttendanceLogs, overviewDateRange[0].month() + 1, overviewDateRange[0].year(), overviewPackingLogs, packingCommission, employmentEndDates, payrollOverrides, overviewAttendanceLogsReady, activeSalesBonusSummary.bonusAmount
        );
    ...
    const matchesCurrentUserPayrollRow = useCallback((row: { username?: string; name?: string }) => {
    ...
    const renderFineAmount = (value: number, ready: boolean, className = 'att-money-red') => (
        ready
            ...
            : <Tooltip title="Phạt tạm tính; hệ thống vẫn đang bổ sung các nguồn tự động">
                <span className={`${className} att-money-pending`}>
                    {formatDeductionAmount(value)} <SyncOutlined spin aria-label="Đang cập nhật" />
                </span>
            </Tooltip>
    );
    const renderOverviewTotalAmount = (value: number, ready: boolean) => (
        ready ? (
            <Tooltip title={fmt(value || 0)}>
                <span className="att-overview-total-money__label">{fmt(value || 0)}</span>
            </Tooltip>
...
const openConfigModal = () => {
        ...
        bonusForm.validateFields().then(values => {
            ...
            if (editingBonus) {
                ...
                const newBonus: BonusRecord = {
                    ...
                    createdBy: fineAuditActor.username,
                    createdByName: fineAuditActor.displayName,
                    ...overtimeFields,
                };
    ...
    }, [bonusForm, editingBonus, employees, canManageBonuses, fineAuditActor]);
    ...
    }, [employees, canManageBonuses, fineAuditActor]);
    ...
    const lockPayroll = () => {
        if (!isAdmin) {
            message.error('Chỉ Admin được phép chốt bảng lương.');
            return;
        }
        if (!isPayrollDataReady) {
            message.info(packingLoadError || 'Đang tổng hợp đầy đủ dữ liệu chấm công, đóng gói và các khoản phạt. Vui lòng chờ trước khi chốt kỳ.');
            return;
        }
        if (isCurrentPeriodLocked) {
                        ...
                        <Table.Summary.Row className="att-overview-total-row">
                            ...
                            <Table.Summary.Cell index={7} colSpan={2} align="right" className="att-overview-total-cell-final">
                                <span className="att-overview-total-money">
                                    {renderOverviewTotalAmount(totalFinalSalary, isPayrollDataReady)}
                                </span>
                            </Table.Summary.Cell>
                        </Table.Summary.Row>
                    </Table.Summary>
    ...
    const renderPackaging = () => {
        ...
        const selectedRangeIsThisWeek = packingDateRange[0].isSame(weekStart, 'day')
            && packingDateRange[1].isSame(weekEnd, 'day');
        const selectedRangeIsFullMonth = packingDateRange[0].isSame(packingDateRange[0].startOf('month'), 'day')
            && packingDateRange[1].isSame(packingDateRange[0].endOf('month'), 'day');
        const selectedRangeLabel = selectedRangeIsThisWeek
            ? `tuần ${weekStart.format('DD/MM')}–${weekEnd.format('DD/MM/YYYY')}`
            : selectedRangeIsFullMonth
                ? `tháng ${packingDateRange[0].format('MM/YYYY')}`
                : packingDateRange[0].isSame(packingDateRange[1], 'day')
                    ? `ngày ${packingDateRange[0].format('DD/MM/YYYY')}`
                    : `${packingDateRange[0].format('DD/MM')}–${packingDateRange[1].format('DD/MM/YYYY')}`;
        const completedWeeklyAwards = packingWeeklyResults.filter(result =>
            result.completed
            && result.leader
            && result.units > 0
            && !result.weekEnd.isBefore(packingDateRange[0], 'day')
            && !result.weekEnd.isAfter(packingDateRange[1], 'day')
        );
        // Keep the previous winner visible even when the active filter is this week.
        const latestCompletedWeeklyAward = liveWeeklyPackingResults
            .filter(result => result.completed && result.leader && result.units > 0 && result.weekEnd.isBefore(weekStart, 'day'))
            .slice(-1)[0];
        ...
        return (
            <div className="packing-league">
                <section className="packing-duel" aria-label="Bảng đua số đơn đóng gói">
                    <header className="packing-duel__header">
                        <div className="packing-duel__title"><TrophyOutlined /><h3>Số đơn hàng</h3></div>
                        <button type="button" className="packing-duel__live" onClick={reloadPacking}><span /> Cập nhật realtime</button>
                    </header>

                    <div className="packing-duel__lanes">
                        {[leader, runnerUp].map((entry, index) => {
                            const maxOrders = Math.max(leader?.orderCount || 1, runnerUp?.orderCount || 1);
                            const progress = entry ? Math.max(22, Math.round(entry.orderCount / maxOrders * 91)) : 22;
                            return (
                                <article key={entry?.id || `empty-${index}`} className={`packing-duel__lane packing-duel__lane--${index === 0 ? 'leader' : 'runner'}`}>
                                    <div className="packing-duel__avatar"><img src={index === 0 ? packingRaceLeaderFox : packingRaceRunnerPanda} alt={`Linh vật Top ${index + 1}`} /></div>
                                    <div className="packing-duel__identity">
                                        <div className="packing-duel__person-row">
                                            <strong>{entry?.name || 'Chưa có dữ liệu'}</strong>
                                            <span className="packing-duel__status">{index === 0 ? <CrownOutlined /> : <ThunderboltOutlined />}{index === 0 ? 'ĐANG DẪN ĐẦU' : 'ĐANG BÁM ĐUỔI'}</span>
                                        </div>
                                        <small>@{entry?.username || '—'}</small>
                                        <div className="packing-duel__track">
                                            <span className="packing-duel__track-fill" style={{ width: `${progress}%` }}>
                                                <span className="packing-duel__chevrons">{[0, 1, 2, 3, 4].map(value => <RightOutlined key={value} />)}</span>
                                                <i />
                                            </span>
                                            <FlagOutlined className="packing-duel__flag" />
                                        </div>
                                    </div>
                                    <div className="packing-duel__score">
                                        <div><span className="packing-duel__digits" key={`${entry?.id || index}-${entry?.orderCount || 0}`}>{renderOdometer(entry?.orderCount || 0)}</span><b>đơn</b></div>
                                        <small>+1 vừa cập nhật</small>
                                    </div>
                                </article>
                            );
                        })}
                        <div className="packing-duel__gap">
                            <LeftOutlined /><div><strong>{Math.max(0, (leader?.orderCount || 0) - (runnerUp?.orderCount || 0))} đơn</strong><span>Khoảng cách {Math.max(0, (leader?.orderCount || 0) - (runnerUp?.orderCount || 0))} đơn</span></div><RightOutlined />
                        </div>
                    </div>

                    <div className="packing-duel__footer">
                        <article className="packing-duel__income">
                            <div className="packing-duel__money-icon"><DollarOutlined /></div>
                            <div className="packing-duel__income-main"><span>Tổng thu nhập sản lượng</span><strong>{fmt(totalIncome)}</strong></div>
                            <div className="packing-duel__income-meta"><span><InboxOutlined /> {totalUnits.toLocaleString('vi-VN')} SP đã ghi nhận</span><i /><span><TagsOutlined /> Đơn giá mặc định: <b>{unitPrice}đ/SP</b></span></div>
                        </article>

                        <article className="packing-duel__reward">
                            <div className="packing-duel__reward-current">
                                <CalendarOutlined className="packing-duel__calendar" />
                                <div>
                                    <h4>Thưởng tuần <strong>{fmt(PACKING_WEEKLY_REWARD_AMOUNT)}</strong></h4>
                                    <span className="packing-duel__date">{weekStart.format('DD/MM')} – {weekEnd.format('DD/MM')}</span>
                                    <p><InboxOutlined /> {currentWeeklyResult?.orderCount.toLocaleString('vi-VN') || 0} đơn · {currentWeeklyResult?.units.toLocaleString('vi-VN') || 0} SP</p>
                                    <p><SyncOutlined /> Tự động reset Thứ Hai</p>
                                    <b className="packing-duel__leading"><TrophyOutlined /> {weeklyLeader ? `Đang dẫn đầu: ${weeklyLeader.name}` : 'Đang chờ dữ liệu'}</b>
                                </div>
                            </div>
                            {latestCompletedWeeklyAward?.leader ? (
                                <div className="packing-duel__previous">
                                    <img className="packing-duel__previous-title" src={packingRaceWinnerHeader} alt="Vô địch tuần trước" />
                                    <strong>{latestCompletedWeeklyAward.leader.name} · {latestCompletedWeeklyAward.orderCount.toLocaleString('vi-VN')} đơn · +{fmt(PACKING_WEEKLY_REWARD_AMOUNT)}</strong>
                                    <div><span>{latestCompletedWeeklyAward.weekStart.format('DD/MM')} – {latestCompletedWeeklyAward.weekEnd.format('DD/MM')}</span><b><CheckCircleOutlined /> Đã nhận thưởng</b></div>
                                </div>
                            ) : <div className="packing-duel__previous packing-duel__previous--empty"><TrophyOutlined /><span>Chưa có kết quả tuần trước</span></div>}
                        </article>
                    </div>
                </section>

                <div className="packing-league-layout">
                    <div className="packing-league-main">
                        <section className="packing-panel">
                            <div className="packing-panel-title">
                                <div><BarChartOutlined /> Bảng xếp hạng hiệu suất · {selectedRangeLabel}</div>
                            ...
                            <div className="packing-ranking-scroll">
                                <table className="packing-ranking-table">
                                    <thead><tr><th>Hạng</th><th>Nhân viên</th><th>Đơn</th><th>Sản phẩm</th><th>Thu nhập</th><th>Tỷ trọng</th><th>Xu hướng<br /><small>so với kỳ trước</small></th></tr></thead>
                                    <tbody>
                                        {leaderboard.map((entry, index) => {
                                            ...
                                            return <tr key={entry.id} className={`packing-rank-row packing-rank-${index + 1}`}>
                                                ...
                                                <td><span className={entry.trend >= 0 ? 'packing-up' : 'packing-down'}>{entry.trend >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />} {entry.trend >= 0 ? '+' : '−'}{Math.abs(entry.trend)} đơn</span></td>
                                            </tr>;
                                        })}
                                    </tbody>
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Text type="secondary" style={{ fontSize: 12 }}>{packingDateRange[0].format('DD/MM/YYYY')} — {packingDateRange[1].format('DD/MM/YYYY')}</Text>
                    <Button size="small" icon={<SyncOutlined />} onClick={() => loadPackingOrders(undefined, { range: packingDateRange, includeComparison: true })}>Tải lại</Button>
                </div>
    ...
    const renderAdditionalBonuses = () => {
        ...
        if (bonusView === 'personal' || !canManageBonuses) {
            ...
            const personalBonuses = normalizedSearch
                ...
                : allPersonalBonuses;
            const personalTotal = allPersonalBonuses.reduce((sum, bonus) => sum + bonus.amount, 0);
            const latestBonusDate = allPersonalBonuses.find(bonus => bonus.date && dayjs(bonus.date).isValid())?.date;

            return (
                <div className="att-personal-bonus">
                    <div className="att-personal-bonus-head">
                        <div>
                            <Title level={3} style={{ margin: 0 }}>Thưởng của tôi</Title>
                            <Text type="secondary">Các khoản thưởng được ghi nhận cho bạn</Text>
                        </div>
                        <div className="att-personal-bonus-actions">
                            ...
                            />
                            {canManageBonuses && (
                                <Button icon={<SettingOutlined />} onClick={() => setBonusView('manage')}>
                                    Quản lý thưởng
                                </Button>
                            )}
                        </div>
                    ...
                    <div className="att-personal-bonus-summary">
                        <div className="att-personal-bonus-stat att-personal-bonus-stat--total">
                            ...
                            <div>
                                ...
                                <strong>{fmt(personalTotal)} đ</strong>
                            </div>
                        </div>
                        <div className="att-personal-bonus-stat">
                            <GiftOutlined />
                            <strong>{allPersonalBonuses.length}</strong>
                            <span>khoản thưởng</span>
                        </div>
                        <div className="att-personal-bonus-stat">
                            <ClockCircleOutlined />
                            <div>
                                <span>Cập nhật gần nhất</span>
                                <strong>{latestBonusDate ? dayjs(latestBonusDate).format('DD/MM/YYYY') : '—'}</strong>
                            </div>
                        </div>
                    </div>

                    <Table
                        className="att-personal-bonus-table"
                        rowKey="id"
                        dataSource={personalBonuses}
                        pagination={personalBonuses.length > 10 ? { pageSize: 10, size: 'small' } : false}
                        scroll={{ x: 860 }}
                        locale={{
                            emptyText: currentEmployee
                                ? (normalizedSearch ? 'Không tìm thấy khoản thưởng phù hợp' : 'Chưa có khoản thưởng nào trong kỳ này')
                        ...
                        columns={[
                            ...
                            },
                            {
                                ...
                                dataIndex: 'type',
                                width: 220,
                                render: (type: string) => <Tag className="att-personal-bonus-source">{type?.replace(/\s*\(Admin\)$/i, '') || 'Thưởng'}</Tag>,
                            },
                            {
                                title: 'Nội dung',
                                dataIndex: 'detail',
                            ...
                            },
                            {
                                ...
                                key: 'creator',
                                width: 210,
                                render: (_: unknown, bonus: BonusRecord) => creatorLabel(bonus),
                            },
                            {
                                title: 'Số tiền',
                                dataIndex: 'amount',
                        ...
                        summary={() => personalBonuses.length > 0 ? (
                            <Table.Summary.Row>
                                <Table.Summary.Cell index={0} colSpan={4}><Text strong>Tổng cộng</Text></Table.Summary.Cell>
                                <Table.Summary.Cell index={4} align="right">
                                    <Text strong style={{ color: '#00ab56' }}>+{fmt(personalTotal)} đ</Text>
                                </Table.Summary.Cell>
                            </Table.Summary.Row>
                        ) : null}
                    />

                    <div className="att-personal-bonus-note">
                        <span><LockOutlined /></span>
                        Bạn chỉ có thể xem các khoản thưởng của mình
                    </div>
                </div>
            );
        }
    ...
    const renderFines = () => {
        ...
        const findFineWaiver = (fine: FineRecord) => fineWaivers.find(item =>
            (fine.id && item?.fine?.id === fine.id)
            || (fine.attendanceLogId && item?.fine?.attendanceLogId === fine.attendanceLogId)
        ...
        const liveAttendanceLogIds = new Set(
            [...extraFines, ...autoVatOverdueFines, ...autoDeadlineOverdueFines, ...autoEvidenceOverdueFines, ...autoStockCheckMissingFines]
        ...
        const combinedFinesRaw = [
            ...
            ...extraFines
                ...
                .map(({ fine, manualIndex }) => {
                    const waiver = findFineWaiver(fine);
                    return waiver
                        ? systemFineRow(fine, fine.id || `waived-persisted-${manualIndex}`)
                        : { ...fine, key: fine.id || `manual-${manualIndex}`, empName: employees.find(e => e.id === fine.empId)?.name, isManual: true, manualIndex, source: fine.source };
                })
                .filter(Boolean),
            ...autoVatOverdueFines
                .filter(f => inOverviewRange(f.date))
            ...
            ...fineWaivers
                .filter(waiver => inOverviewRange(waiver.fine?.date)
                    ...
                    && (!waiver.fine?.attendanceLogId || !liveAttendanceLogIds.has(String(waiver.fine.attendanceLogId))))
                .map((waiver, i) => ({
                    ...waiver.fine,
                                    ...
                                    if (record.source === 'attendance' && lateMatch) {
                                        ...
                                        return (
                                            <div style={fineDetailCellStyle}>
                                                ...
                                                {record.isWaived && (
                                                    <Text style={{ display: 'block', marginTop: 4, color: '#237804', fontSize: 11, fontWeight: 700 }}>
                                                        Lý do miễn: {record.waiverReason || `Đạt chuỗi đúng giờ 7 ngày liên tiếp; lần đi muộn ${minutes} phút thuộc mức được miễn.`}
                                                    </Text>
                                                )}
                                            </div>
                                        );
    ...
    const persistLeaveStatus = useCallback(async (
        emp: any,
        date: dayjs.Dayjs,
        ...
        const status = action === 'clear'
            ...
            : action === 'unpaid'
                ...
                : action === 'exempt'
                    ? 'exempt'
                    : 'paid';
        const result = await window.electronAPI.attendance.updateLeaveStatus({
            empId: Number(emp.id),
            date: date.format('YYYY-MM-DD'),
            sessions,
            status,
        ...
        if (!result?.success) {
            throw new Error(result?.error || 'Không lưu được trạng thái nghỉ.');
        }
        const nextRecords = Array.isArray(result.data?.leaveRecords) ? result.data.leaveRecords : [];
        setLeaveRecords(nextRecords);
        if (Array.isArray(result.data?.extraFines)) {
            setExtraFines(result.data.extraFines.map((fine: FineRecord, index: number) => ensureFineId(fine, index)));
        }
        if (Array.isArray(result.data?.fineWaivers)) setFineWaivers(result.data.fineWaivers);
        if (Array.isArray(result.data?.fineAuditLog)) setFineAuditLog(result.data.fineAuditLog);
        setAttendanceRewardRefreshKey(value => value + 1);
        return nextRecords;
    }, []);

    const saveLeaveRequestInline = useCallback(async (
        emp: any,
        date: dayjs.Dayjs,
        action: 'save' | 'clear' | 'unpaid' | 'exempt',
        scope: LeaveSession | 'full_day',
        ...
        try {
            await persistLeaveStatus(emp, date, action, scope, note);
            ...
            message.error(error?.message || 'Không lưu được trạng thái nghỉ.');
        }
    }, [canManageAttendance, canManageAttendanceEmployee, canEditAttendanceDate, checkLocked, persistLeaveStatus]);
    ...
    const openLeaveRequestModal = useCallback((emp: Employee, date: dayjs.Dayjs, defaultSession: LeaveSession, existingLeave?: LeaveRequest) => {
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    await persistLeaveStatus(emp, date, selectedAction === 'clear' ? 'clear' : 'save', selectedScope, note);
                    message.success(selectedAction === 'clear' ? 'Đã xóa lịch xin nghỉ.' : 'Đã lưu lịch xin nghỉ.');
                } catch (error: any) {
                    message.error(error?.message || 'Không lưu được lịch xin nghỉ.');
                }
            },
        });
    }, [canManageAttendance, canManageAttendanceEmployee, canEditAttendanceDate, checkLocked, persistLeaveStatus]);
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                if (selectedAction === 'leave') {
                    try {
                        await persistLeaveStatus(emp, date, 'save', selectedScope, note);
    ...
    }, [canManageAttendanceEmployee, canManageWorkSchedules, canEditAttendanceDate, checkLocked, currentUser, persistLeaveStatus, user?.username]);
    ...
    const currentAttendanceEmployee = useMemo(() => employees.find(emp => {
        const loginUsername = normalizeAttendanceText(user?.username || currentUser || '');
        const loginFullName = normalizeAttendanceText(user?.fullName || '');
        return normalizeAttendanceText(emp.username) === loginUsername
            || (loginFullName && normalizeAttendanceText(emp.name) === loginFullName);
    }), [currentUser, employees, user?.fullName, user?.username]);
    const currentAttendanceReward = useMemo(() => (
        currentAttendanceEmployee
                                    ...
                                    {payslipPdfDetailOpen ? (
                                        <>
                                            {isAdmin && (isCurrentPeriodLocked ? (() => {
                                                ...
                                                return sentTime ? (
                                                    <Tooltip title={`Gmail đã tiếp nhận lúc ${sentTime} — Bấm để gửi lại`}>
                                                        <Button
                                                            ...
                                                            onClick={() => Modal.confirm({
                                                                ...
                                                                content: `Gmail đã tiếp nhận phiếu lúc ${sentTime}. Bạn có muốn gửi lại không?`,
                                                                okText: 'Gửi lại',
                                                                cancelText: 'Hủy',
                        ...
                        <div>
                            <div style={{ marginBottom: 16, textAlign: 'center' }}>
                                <Text style={{ fontSize: 15 }}>
                                    Gmail đã tiếp nhận:{' '}
                                    <strong style={{ color: '#52c41a' }}>{bulkSendProgress.results.filter(r => r.success).length}</strong> thư,{' '}
                                ...
                                <div style={{ marginTop: 6, color: '#8c8c8c', fontSize: 12 }}>
                                    Thư bị máy chủ người nhận trả lại sẽ xuất hiện sau trong hộp thư Gmail và không thể xác nhận chỉ bằng quyền gửi.
                                </div>
                            </div>
